...
一定のルールで正規化した値を取得したい場合にヒューリスティックパーサは便利である。
...
例えば次のyamlは文字列から月給の下限と上限を取得するparserを実装する例。
...
「yamlのinputとexpectedに合うparserを自作して、vitestで実装して」とAIに命令すれば良い。
