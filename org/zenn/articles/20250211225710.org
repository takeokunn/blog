...
Emacsは独特のキーバインドが特徴的ですが、私とtadsanにとってはEmacsはあくまでLispマシンとして機能してくれればいいのでキーバインドについてあまり触れませんでした。
結局Emacsの魅力を伝えるには細かいキーバインドや機能よりもユースケースやライフスタイルなのでは、というのが最近の私の感想です。
