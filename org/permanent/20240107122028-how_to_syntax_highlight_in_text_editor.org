...
シンタックスハイライトには、「テキストの可読性を向上させ文脈をより明瞭にする」や「記述ミスや括弧の対応のミスなどを防ぐことができる」等、様々なメリットがあります。
...
Tree-sitterはもともとAtomで使用するためにGitHubによって開発され、2018年に初めてリリースされました。
...
例えばVueJSはHTML/CSS/JavaScriptを1ファイル内で記述できるし、PHPも元々HyperText PreprocessorなのでHTMLを記述することができます。
...
EmacsLispの正規表現がそもそも難しいのもあり、複雑怪奇で特殊な訓練しないと読めないことがわかるでしょう。
