...
GPG Keyの運用についてかねてから悩んでいたが、YubiKey運用が一番良いという結論に至った。
丁度Macを初期化するタイミングがあったので完全に移行した。

以下の記事を参考にした。

- [[https://zenn.dev/lambdalisue/articles/gpg-with-yubikey-2024][GPG 鍵と YubiKey の運用メモ - Zenn]]
- [[https://keens.github.io/blog/2021/03/23/yubikeywotsukau_openpghen/][YubikeyでOpenPGP鍵をセキュアに使う]]
- [[https://fuwa.dev/posts/yubikey/][YubiKey(OpenPGP card)には主鍵を入れよう]]

* 試したこと・やったこと
** 0. 運用方針を決める

[[https://zenn.dev/lambdalisue/articles/gpg-with-yubikey-2024][GPG 鍵と YubiKey の運用メモ - Zenn]] と同様、以下の方針で運用してくことにした。
...
gpg-agentを有効にして =~/.gnupg/gpg-agent.conf= に以下のような設定をした。
...
[[id:A942A0CA-829F-45C0-A9CC-F7CA8C0DE873][Terraform GitHub Providerを導入した]] の通り、terraform経由でssh keyを登録する。
...
GPG keyのpublic keyは変化ないので初回登録しておけば良い。

https://github.com/takeokunn.keys

* 得られた結果・所感

想像以上にスムーズにYubiKeyに移行することができた。
