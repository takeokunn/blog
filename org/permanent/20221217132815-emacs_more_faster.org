...
日々HEAD buildをしているのでstableのversionとは微妙に差異がありますが、起動速度にはそこまで大きな差が出ないでしょう。
...
Sample数が素に近ければ近いほど、高速に動いていると言えます。
...
私は ~use-package~ には疎いので正確なことは書けませんが、~s式~ の効率が良いかと言われたら最高速を叩き出せるものではありません。
